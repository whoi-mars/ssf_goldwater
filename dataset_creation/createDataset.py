...
X = []
nums = []
for row in range(len(labels))[:20]:
    ...
    starti, stopi = spect.range_to_indices(startTime - buffer + timeShift, endTime + buffer + timeShift, Fs)
    samples = samples_norm[starti:stopi, channel]
    _, _, Zxx, fs = spect.my_stft(samples, Fs, window_N, window_overlap=10, NFFT=2 ** 8, DECIMATE_FACTOR=2)
    Zxx = Zxx[round(Zxx.shape[0] / 2):, :]
    spectro = 10 * np.log10(np.abs(Zxx) ** 2)

    # length = spectro.shape[1]
    # if length != 128:
    #     spectro = spectro[:, 64:length - 63]

    X.append(spectro)
    nums.append(file.split('_')[-1].split('.')[0])
